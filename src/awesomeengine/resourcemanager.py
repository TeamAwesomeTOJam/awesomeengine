...
from awesomeengine.jsonparser import JSONParser
from awesomeengine.geometry import Vec, Rect


JSON_PARSER = JSONParser()
JSON_PARSER.add_object_type(Vec)
JSON_PARSER.add_object_type(Rect)
...
def LoadEntityData(prefix, key):
    ...
    return JSON_PARSER._freeze_value(definition)
...
def LoadAnimation(prefix, key):
    with open(os.path.join(prefix, 'animations', key + '.json')) as in_file:
        return JSON_PARSER.parse_file(in_file)
